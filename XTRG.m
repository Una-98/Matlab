function [taus,lnZs,rho,Ovals] = XTRG (Hs,dt,tmax,O,Nkeep,Nsweep)
% < Description >
%
...
Nstep = round(log2(tmax)-log2(dt)); % MPO multiplications to reach beta
taus = dt*(2.^(1:Nstep)); % inverse temperatures where observables are evaluated
...
for it1 = (1:Nstep)
    ...
    for itN = (1:N)
        ...
        O1=contract(Tr,4,[3 2],O,2,[1 2]);
        for it2=itN+1:N
            O1=contract(O1,2,2,rho{it2},4,3);
            O1=contract(O1,4,[2 3],getIdentity(O1,3),2,[2 1]);
        end
