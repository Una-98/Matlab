function Cright = updateRight(Cright,rankC,B,X,rankX,A)
% < Description >
%
...
if (isempty(rankC) || (rankC < ndims(Cright))) || ...
    ...
    if ~isempty(Cright)
        if (rankC > 2) && (rankX > 2)
            if rankX == 4 % (rankC,rankX) = (3,4)
                ...
                T = contract(Cright,rankC,[rankC 2],T,rankX+1,[2 rankX+1]);
                Cright = contract(B,3,[2 3],T,rankC+rankX-3,[1 2],[1 3 2]);
            else % (rankC,rankX) = (3,3)
                % contract the operator-flavor legs of Cleft and X
                T = contract(Cright,rankC,[rankC 2],T,rankX+1,[2 rankX+1]);
                Cright = contract(B,3,[2 3],T,rankC+rankX-3,[1 2]);
            end
        else % (rankC,rankX) = (2,2), (2,3), (3,2)
            T = contract(Cright,rankC,2,T,rankX+1,rankX+1);
            Cright = contract(B,3,[2 3],T,rankC+rankX-1,[1 rankC],[1 (3:rankC+rankX-2) 2]);
        end
    elseif (rankX == 4) && (size(X,3) == 1) % no Cleft, rankX = 4
        % if X is rank-4 and its left leg is dummy, and Cleft is empty (i.e., identity)
        Cright = contract(B,3,[2 3],T,rankX+1,[rankX+1 1],[1 4 3 2]);
        % permute the left dummy leg of X to be at the end; to be ignored
        % as singleton dimension
    else % no Cleft, rankX = 2,3
        Cright = contract(B,3,[2 3],T,rankX+1,[rankX+1 1],[1 (3:rankX) 2]);
    end
elseif ~isempty(Cright) % no X, rankC = 2, 3
    T = contract(Cright,rankC,2,A,3,2);
    Cright = contract(B,3,[2 3],T,rankC+1,[1 rankC+1],[1 (3:rankC) 2]);
else % no Cleft, no X
    Cright = contract(B,3,[2 3],A,3,[2 3]);
end
